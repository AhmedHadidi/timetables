...
6. **أنشئ حساب المدير:** من **Authentication → Users → Add user** أدخل:
...
يمكن أن يكون لديك أكثر من مدير واحد. لإضافة مدير جديد:
1. أنشئ له حسابًا في **Supabase → Authentication → Users → Add user** (بريده وكلمة مرور، مع تفعيل Auto Confirm).
2. افتح `src/lib/config.ts` وأضف بريده داخل مصفوفة `ADMIN_EMAILS`:
   ```ts
   export const ADMIN_EMAILS: string[] = [
     'seebawy19@gmail.com',
     'admin2@example.com',   // أضف بريد المدير الجديد هنا
   ];
   ```
3. احفظ، وادفع التغيير إلى GitHub (`git add . && git commit -m "إضافة مدير" && git push`) — Vercel يعيد النشر تلقائيًا.

⚠️ **مجرد إنشاء حساب في Supabase لا يكفي وحده** — يجب أيضًا إضافة بريده في `ADMIN_EMAILS`، وإلا سيسجّل دخوله بنجاح لكن يبقى في وضع العرض فقط (هذا هو الحماية المقصودة: التحكم بالصلاحية من الكود لا من قاعدة البيانات فقط).
...
- **المديرون فقط** (القائمة في `ADMIN_EMAILS`) تظهر لهم أدوات: إنشاء الجدول، تعديل الخلايا، حفظ التعديلات، إضافة/حذف الموظفين، الإعدادات.
